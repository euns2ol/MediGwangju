...
class MapViewContainer: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate {
    ...
    func hospitalAlert(hospitalItem: Array<Hospital>) -> Void{
        ...
        alert = UIAlertController(title: NSLocalizedString("병원 선택", comment: ""), message:nil, preferredStyle: .alert)
        
        //컨트롤 뷰 컨트롤러를 알림창에 등록한다.
        alert!.setValue(contentVC, forKey: "contentViewController")
        
        let okAction = UIAlertAction(title: NSLocalizedString("취소", comment: ""), style: .default)
...
//        print("근접한 병원이 없네요")
    ...
    func didTapMyLocationButton(for mapView: GMSMapView) -> Bool {
        ...
        if(checkGPS == false) {
            let message = NSLocalizedString("위치정보를 이용할 수 없습니다.", comment: "")
