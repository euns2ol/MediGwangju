...
class ViewController: UIViewController, XMLParserDelegate{
    ...
    private func setupNavigationBarItems() {
        navigationItem.title = "Medi-Gwangju"
        ...
        infoButton.frame = CGRect(x: 0, y: 0, width: 15, height: 15)
        infoButton.contentMode = .scaleAspectFit
        navigationItem.leftBarButtonItem = UIBarButtonItem(customView: infoButton)
        
        infoButton.addTarget(self, action: #selector(ViewController.infoButtonAction), for: .touchUpInside)
    }
    
    @objc func infoButtonAction(_ sender: UIButton) {
        let selectView: informationController = UIStoryboard(name: "informationView", bundle: nil).instantiateViewController(withIdentifier: "informationView") as! informationController
